...
Track your lifts, log every session, watch your body composition and strength trend over time, earn achievement badges and ranks as you go, and get push notifications for rest timers and measurement reminders — even if the app isn't open.
...
This app is a 4-day training program tracker (**Push / Pull / Legs / Whole Body**) with per-exercise weight tracking, a training calendar, body measurement logging, progress charts, a diet calculator, a home dashboard, and a full rank/achievement system — all running client-side in the browser, with data stored locally on your device.

It's designed to be installed as a **Progressive Web App (PWA)**: add it to your Home Screen once, and it opens full-screen like a native app, works offline, walks first-time users through a short onboarding, and can send you push notifications for rest timers and periodic measurement reminders.
...
- A round **⏱️ rest timer** per exercise — keeps counting even if you switch tabs, edit an exercise elsewhere, or background the app, and beeps at 30 seconds remaining and at zero. The countdown panel and its beeps survive re-renders (like editing a different exercise) instead of silently reverting.
- While counting down, the tile shows the set you just finished (e.g. **"I DONE"**) plus that exercise's planned sets × reps, then advances to the next set number (**II, III…**) once the timer ends — no need to open Tips to check your rep scheme mid-set.
- **💪 Done** button on the last exercise logs the session to the calendar and opens a **session summary**: an estimated volume range from the exercises you actually timed (with the exact weight behind every new record), any new SB/PB records, and anything flagged with Increase — plus a matching-colour confetti celebration. Logging a session with a different type than what was planned replaces the plan with what you actually did, keeping only its Deload flag.
- **Whole Body is special**: every exercise there is a *live link* to one already defined in Push, Pull, or Legs — not a separate copy. Logging a weight, editing notes, or updating Tips updates it everywhere at once. Adding an alternative to Whole Body opens a picker of your existing exercises instead of a blank form (Abs excluded, since it links automatically).
...
### 💡 Tips
- Full-screen view per exercise with LT/SB/PB, the rest timer (highlighted orange while running), and a rich-text notes editor (bold, underline, bullet lists).
...
- Optional YouTube link field for a form-check video, opened via a real link tap so returning from YouTube lands you straight back on the workout instead of a blank in-app browser screen.
...
- Resetting SB/PB or logging a new LT updates every pill and the Increase flag on the spot — no need to leave and reopen Tips to see the change.

### 📅 Calendar
- Log a completed session or plan a future one by tapping any day — planning only asks for the training type, an optional Deload flag, and a note, since diet and cardio are same-day facts.
- Once the date arrives, mark a planned day Done or cancel it.
- Mark any completed session as a **Deload** week (shown with a small 🔻 on the calendar).
- **🥗 No Cheat Meal** — two checkboxes on a logged day: one for a clean day where you also trained, and one for a clean day with no training at all. Checking the training-less option grays out the training type and deload fields and lets you log the day without picking a session type. A clean-only day stays visually neutral on the grid (just the 🥗 mark), so the calendar's colours stay focused on training status.
- **🏃 Post-Session Cardio** can be attached to any logged day from the same form, or with the one-tap green **Mark Today as a Clean Day** button above the grid.
- The month grid is a real-time swipeable carousel — it follows your finger and springs back if you don't drag far enough, revealing the neighbouring month as you go; the ‹ › arrows still jump instantly.
- The entry list below supports swipe-to-edit and swipe-to-delete; tapping a completed session instead slides open a **📊 View Details** button leading to that session's summary (or a plain "no details" note if it predates this feature).
...
- Last completed session — tap it to reopen that session's summary — and next planned one (tap to jump straight to that tab), each showing how long ago / how soon.
- Weekly consistency streaks (weeks with 3+ and 2+ sessions in a row).
- Tappable **Total Training Days** (→ Calendar) and **Last Deload** (→ that month) tiles, each with a subtle › hinting they're interactive.
...
- The first launch after fully closing the app (not just switching away) animates the streak numbers, training-day total, and consistency ring counting up from zero instead of appearing instantly.

### 🥗 Diet
- **Clean Diet Days** streak and total, and a one-tap **Mark Today as a Clean Day** button, shown right at the top — followed by a Clean Days per month chart.
- Katch-McArdle based daily calorie and macro targets from age, weight, height, body fat %, training frequency, and a **Daily Activity Level** (Low/Medium/High, for non-training movement) — separate targets for maintenance, lean bulk, and cut.
- Age and height auto-fill from your **Profile**; weight and body fat from your latest **Body Stats** entry — a banner offers to update your targets if your Body Stats have since drifted from what you last calculated with.
...
### 🏃 Post-Session Cardio
- Every training tab ends with a swipeable cardio card — browse saved options by swipe or ‹ ›, each labelled with the training phase it suits (e.g. "Cut / Fat Loss"), a duration, and a short description.
- **＋ Add** creates a new option; **✏️ Edit** changes or deletes (with confirmation) the one currently showing.
- **🏃 Cardio Completed** sends a burst of running emoji across the screen and marks the day with a small 🏃‍♂️ on the calendar and its entry — but only once a real strength session is logged for today; otherwise it offers a **Log Cardio Training** shortcut to log a standalone Cardio session instead.

### 🎖️ Profile, Rank & Achievements
- Tap the round avatar in the header to open your Profile.
- Set a **nickname** (type it, tap the ✓ that appears, or hit Enter), **age** and **height** (which auto-fill the Diet calculator), and pick an **avatar** — a coloured preset or your own photo from camera/library, with a remove option.
- An **Accent Color** picker swaps the app's orange highlight for one of seven other colours; training-type, chart, and badge-category colours stay fixed on purpose.
- A **This Week** snapshot of the current calendar week (weekends shaded slightly lighter) sits below that, tappable through to the Calendar.
- **Rank & Achievements** combines your badge count with four rank trophies — 🥉🥈🥇🏆 — laid directly on the shared progress bar at the 25/50/75/100% marks. Thresholds are percentages of the total badge count, so they shift automatically as more badges get added. Reaching a rank re-colours the avatar's header border to match (gold plus flanking trophies for Champion) and pops a distinct, centre-screen celebration with its own coloured confetti burst — separate from the regular badge popup, and always shown after any badge popups from the same action have been dismissed.
- **99 badges** across 10 categories — Training Consistency, Planning, Strength Milestones, Perfect Rests, Discipline, Body Tracking, Personalization, Personal Trainer, Post-Workout Cardio, and Clean Eating. Locked badges show a progress ring around the icon (filled to match how close you are) plus a live count (e.g. "62/100"); unlocked ones glow in their category colour.
- Badges — and ranks — **pop up the moment you earn them**, wherever you are in the app, including from resets and edits in the Record Log or Tips.

### ⚙️ Settings
- **📜 Record Log** — every SB and PB you've ever set, newest first, searchable by exercise name. Tap **☑️ Select** to check off individual records (or Select All) and delete them together, or swipe a row right to edit its weight/date and left to delete it individually; the exercise's current record recalculates automatically from what remains.
- **Export** — all data, or just exercises, calendar, or body stats — as a JSON file.
- **Import** — restore from any of the above exports.
- **Clear by section** — wipe specific data (exercises, weights, SB only, PB only, calendar, body stats, or every unlocked badge) without resetting everything else.
...
- Rest-timer alerts arrive as real push notifications even when the app is closed, via a small relay server + [ntfy.sh](https://ntfy.sh). Both the wake-up ping and the notification request retry with backoff, so a cold, sleeping server doesn't silently swallow the very first notification of a session.
- In-app beeps at 30 seconds and at zero self-heal after the app is backgrounded and restored — the audio engine is verified and, if iOS left it stuck, rebuilt automatically, rather than staying silently dead until the page is reloaded.
...
- A short onboarding walkthrough introduces new users to the app on first launch; replay it any time from the **ℹ️ About** tab.
...
- **Web Audio API** — timer beeps, with self-healing recovery from iOS backgrounding.
...
2. Upload `index.html`, `sw.js`, `apple-touch-icon.png`, and `apple-touch-icon-180.png` to the repo root.
...
3. Launch it from the Home Screen icon — it now runs full-screen, like a native app, with offline support and a short onboarding on first launch.
...
3. In `index.html`, confirm the relay endpoint (in the notification-scheduling functions) points at your deployed Render URL.
...
> **Free tier note:** Render's free plan sleeps after inactivity and can take 30–50 seconds to wake up. Both the app's wake-up ping on launch and its notification-scheduling requests retry with backoff to ride out a cold start, recalculating the remaining delay on each attempt so a late-arriving schedule still fires at the right moment. It's also why the measurement reminder is checked client-side on app open rather than scheduled 24 days in advance — see [Known Limitations](#known-limitations).

---

## Usage Guide

New here? The app has a full **ℹ️ About** page (bottom navigation, paired with Settings) with a section-by-section walkthrough of every feature, plus a button to replay the first-launch onboarding any time — that's the fastest way to get oriented after installing.
...
- **Badges are permanent once earned; ranks are not stored, they're derived** — `checkBadges()` only ever adds to the unlocked set, never removes from it. Rank *thresholds*, however, are percentages of the live badge total and shift as badges are added, so which rank you currently hold is always recomputed from the current badge count rather than read from a saved flag — a flag written under an older threshold is treated as stale and cleared rather than trusted.
- **A background rebuild can silently orphan a running timer's DOM node** — any full re-render of a training day's exercise list (e.g. after editing an exercise) replaces every tile from scratch, so a timer's stored reference to its slide element goes stale. The render function specifically checks for and reconstructs any still-running timer's countdown panel (including its highlighted border) afterward, rather than letting it silently revert to the idle tile.
- **Diet-only calendar days don't count as training** — a day logged as "No Cheat Meal" with no training attached (`dietOnly: true`) is deliberately excluded from every statistic that counts training sessions (streaks, totals, the yearly consistency ring, "last training," etc.), while still counting toward Clean Diet Days stats and badges. The same exclusion mechanism covers a cardio-only fallback day.
- **Session summaries are snapshotted, not recomputed** — the volume/records data shown right after a session is saved (keyed by date) so it can be reopened later from the Dashboard or Calendar even after the raw timer log that computed it is cleared. Badges that depend on session volume are checked *after* this snapshot is saved, not before, so a session can immediately earn a volume-threshold badge on the same session that reached it.
...
- **No custom sound while backgrounded** — iOS suspends a page's audio and JavaScript execution when it's not the active tab/app. The push-notification relay server exists specifically to work around this for rest-timer alerts; in-app beeps use a self-healing AudioContext that verifies (and rebuilds if necessary) that audio genuinely works again after returning to the foreground, since a plain `resume()` call can report success on iOS while output stays silently dead.
